...
static uint32_t grl_pollevent_2epoll(short events) {
    uint32_t e = 0;
    if (events & POLLIN) {
        e |= EPOLLIN;
    }
...
static short grl_epollevent_2poll(uint32_t events) {
    short e = 0;
    if (events & EPOLLIN) {
        e |= POLLIN;
    }
...
/*
 * grl_poll_inner --> 1. sockfd--> epoll, 2 yield, 3. epoll x sockfd
 * fds :
 */

static int grl_poll_inner(struct pollfd *fds, nfds_t nfds, int timeout) {
    if (timeout == 0) {
    ...
    grl_coroutine *co = sched->curr_thread;

    int i = 0;
    for (i = 0; i < nfds; i++) {
        struct epoll_event ev;
        ev.events = grl_pollevent_2epoll(fds[i].events);
        ev.data.fd = fds[i].fd;
        epoll_ctl(sched->poller_fd, EPOLL_CTL_ADD, fds[i].fd, &ev);

        co->events = fds[i].events;
        grl_schedule_sched_wait(co, fds[i].fd, fds[i].events, timeout);
    }
    ...
    for (i = 0; i < nfds; i++) {
        ...
        grl_schedule_desched_wait(fds[i].fd);
    }
...
ssize_t grl_sendto(int fd, void const *buf, size_t len, int flags,
    ...
    while (sent < len) {
        ...
        grl_poll_inner(&fds, 1, 1);
        int ret = sendto(fd, ((char *)buf) + sent, len - sent, flags, dest_addr,
                         addrlen);
        if (ret <= 0) {
            if (errno == EAGAIN) {
                continue;
...
ssize_t send(int fd, void const *buf, size_t len, int flags) {
    if (!send_f) {
        init_hook();
    }
